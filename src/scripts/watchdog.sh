#!/bin/sh
set -eu

# Config
HEARTBEAT_PATH=${HEARTBEAT_PATH:-/app/data/heartbeat.json}
LIVENESS_WINDOW_SEC=${LIVENESS_WINDOW_SEC:-600} # 10 minutes default
GRACE_SEC=${GRACE_SEC:-120} # grace period after startup to allow first heartbeat

# Start app in background (TypeScript build output)
node /app/dist/index.js &
APP_PID=$!

echo "watchdog: started app pid=$APP_PID, monitoring $HEARTBEAT_PATH"

START_TS=$(date +%s)

while true; do
  if ! kill -0 "$APP_PID" 2>/dev/null; then
    echo "watchdog: app exited"
    exit 1
  fi

  NOW=$(date +%s)
  ELAPSED=$((NOW - START_TS))

  if [ -f "$HEARTBEAT_PATH" ]; then
    HB_TS=$(date -r "$HEARTBEAT_PATH" +%s 2>/dev/null || echo 0)
    AGE=$((NOW - HB_TS))
    if [ "$ELAPSED" -ge "$GRACE_SEC" ] && [ "$AGE" -gt "$LIVENESS_WINDOW_SEC" ]; then
      echo "watchdog: heartbeat stale (${AGE}s > ${LIVENESS_WINDOW_SEC}s)"
      kill "$APP_PID" 2>/dev/null || true
      exit 1
    fi
  else
    # If no heartbeat yet, allow grace period before enforcing
    if [ "$ELAPSED" -ge "$GRACE_SEC" ]; then
      echo "watchdog: no heartbeat detected after ${GRACE_SEC}s"
      kill "$APP_PID" 2>/dev/null || true
      exit 1
    fi
  fi

  sleep 30
done


